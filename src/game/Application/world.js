...
export function updateDiscoveredWorld(pixiGame) {
  const world = getAsset(pixiGame.containers.worldFar);
  const { cachedAreaKey, discoveredAreas } = pixiGame;
  const currentAreaCode = getAreaCode(pixiGame.player.data);
  const currentAreaCodeKey = getAreaCodeKey(currentAreaCode);

  if (cachedAreaKey !== currentAreaCodeKey) {
    const outerCodes = getOuterAreaCodes(pixiGame.player.data);
  ...
  pixiGame.cachedAreaKey = currentAreaCodeKey;
}
